...
Teclado/mouse atua no computador e na janela que estiver em foco, não fica restrito à janela capturada. O anfitrião deve deixar o jogo em foco. Não há acesso persistente, execução de comandos, transferência de arquivos nem instalação de drivers por este recurso.
...
| Controle virtual no anfitrião | Ainda não disponível nesta entrega | Um dispositivo uinput por jogador, até quatro |

O backend Windows de controle virtual permanece como próxima etapa; não há dependência nem instalação de ViGEmBus. A interface `VirtualGamepadBackend` permite adicionar outro driver sem mudar o protocolo. Um participante Windows pode usar seu controle para jogar em um anfitrião Linux compatível.
...
No Windows, SendInput respeita o nível de integridade do processo. Jogos elevados ou que recusam input sintético podem não aceitar os comandos. Esta implementação não contorna essas restrições. Referências: [Microsoft SendInput](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-sendinput), [Linux uinput](https://docs.kernel.org/input/uinput.html).
...
- Gamepad envia quatro eixos e 17 botões normalizados, aproximadamente 60 vezes por segundo. Teclas fora da tabela compartilhada não são injetadas; Esc fica reservado para as sobreposições e a saída.
