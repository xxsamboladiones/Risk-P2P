...
import { Check, Clipboard, Link2, UserPlus, X } from "lucide-react";
import { api } from "../api";
...
export function P2PInvitePanel({ type, token, displayName, group, initialMode = "create", onComplete }: {
  ...
  async function copy() { if (!state) return; await navigator.clipboard.writeText(state.code); setCopied(true); window.setTimeout(() => setCopied(false), 1_500); }
  ...
  return <div className="p2p-invite">
    ...
    {!state && mode === "join" && <form className="invite-code-form" onSubmit={(event) => void join(event)}><label>Código de convite</label><input value={code} onChange={(event) => setCode(normalizeRiskInviteCode(event.target.value))} placeholder="risk-____-____-____-____" autoComplete="off" maxLength={25}/><button>Conectar por WebRTC</button></form>}
    {state && <div className={`invite-progress ${state.status}`}>
      ...
      {!["accepted", "rejected", "expired", "cancelled", "error"].includes(state.status) && <button className="cancel-invite" onClick={() => void cancel()}>Cancelar convite</button>}
      {["accepted", "rejected", "expired", "cancelled", "error"].includes(state.status) && <button onClick={() => { setState(undefined); setRequest(undefined); }}>Novo convite</button>}
    </div>}
