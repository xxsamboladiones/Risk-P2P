...
export type InviteStatus = "idle" | "waiting" | "connecting" | "connected" | "approval" | "confirming" | "accepted" | "rejected" | "expired" | "cancelled" | "error";
...
export class InviteService {
  ...
  async joinInvite(type: InviteType, input: string, ttlMs = DEFAULT_INVITE_TTL_MS): Promise<InviteSnapshot> {
    if (!sharedAttemptLimiter.consume()) throw new Error("Muitas tentativas. Aguarde um minuto antes de tentar novamente.");
  ...
  private bindSignaling(): void {
    ...
    this.unsubscribers.push(
      ...
      signaling.onPeerLeft((peerId) => {
        if (peerId === this.candidatePeerId && this.snapshot && !["accepted", "rejected", "expired", "cancelled"].includes(this.snapshot.status)) void this.failCandidate(peerId);
      }),
  ...
  private async receive(raw: string): Promise<void> {
    ...
    if (this.snapshot.role === "creator") {
      if (message.type === "invite.ack" && this.request && message.requestId === this.request.requestId && this.pendingDecision) {
        if (this.pendingDecision === "accept") {
          const remote = this.request.identity;
          if (this.snapshot.type === "friend") await saveLocalFriend({ ...remote, addedAt: this.dependencies.now() });
          else {
            if (!this.group) { this.update("error", "Metadados do grupo indisponíveis."); await this.cleanup(); return; }
  ...
  private async sendAck(requestId: string): Promise<void> {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try { await this.send("invite.ack", requestId); return; }
      catch (error) { lastError = error; if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 150)); }
    }
    throw lastError instanceof Error ? lastError : new Error("Não foi possível confirmar o convite.");
  }

  private armCandidateTimeout(peerId: string): void {
    this.clearCandidateTimeout();
    const remaining = Math.max(1, (this.snapshot?.expiresAt ?? this.dependencies.now()) - this.dependencies.now());
    this.candidateTimer = this.dependencies.setTimer(() => {
      if (this.candidatePeerId === peerId && this.snapshot && ["connecting", "waiting"].includes(this.snapshot.status)) void this.failCandidate(peerId);
  ...
  private armDecisionTimeout(): void {
    ...
    this.decisionTimer = this.dependencies.setTimer(() => {
      if (this.snapshot?.status === "confirming") {
        this.pendingDecision = undefined;
        this.update("error", "O outro dispositivo não confirmou a operação. Tente novamente.");
        void this.cleanup();
      }
  ...
  private async failCandidate(peerId: string): Promise<void> {
    ...
    this.candidatePeerId = undefined; this.request = undefined; this.requestId = undefined; this.pendingDecision = undefined;
  ...
  private async finishSoon(): Promise<void> { await new Promise((resolve) => setTimeout(resolve, 100)); await this.cleanup(); if (typeof window !== "undefined") window.dispatchEvent(new Event("risk:social-updated")); }
  ...
  private async cleanup(): Promise<void> {
    ...
    if (this.candidateTimer) this.dependencies.clearTimer(this.candidateTimer);
    if (this.decisionTimer) this.dependencies.clearTimer(this.decisionTimer);
    this.expiryTimer = undefined; this.availabilityTimer = undefined; this.candidateTimer = undefined; this.decisionTimer = undefined;
    this.unsubscribers.splice(0).forEach((unsubscribe) => unsubscribe());
    await this.transport?.disconnect().catch(() => undefined); await this.signaling?.disconnect().catch(() => undefined);
    this.transport = undefined; this.signaling = undefined; this.localPeerId = undefined; this.candidatePeerId = undefined; this.request = undefined; this.requestId = undefined; this.pendingDecision = undefined;
  }
}
